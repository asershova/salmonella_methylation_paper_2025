...
sites_genome = 2978
meth_type = 'a'
data_08 <- read.csv(paste(working_dir,"LSP-1.GATCAG.bed", sep="/"), sep="", header = FALSE,
...
data_09 <- read.csv(paste(working_dir,"MEP-1.GATCAG.bed", sep="/"), sep="", header = FALSE,
                    col.names = c('chrom', 'm_start', 'm_stop', 'met_type', 'total_reads', 'met_dir', 'met_st_1', 'met_st_2', 'color','reads_all', 'methyl', 'methyl_reads', 'non_methyl_reads',
                                  'v1','v2','v3', 'v4', 'v5', 'chrom2', 'site_start', 'site_stop', 'site', 'v6', 'dir',
                                  'chrom3', 'f_start', 'f_stop', 'f_dir','f_type','locus_tag','id1','id2','locus_tag1','id11','id21','d_chrom','d_start', 'd_stop','d_old','d_id','domain'))
data_10 <- read.csv(paste(working_dir,"MEP-2.GATCAG.bed", sep="/"), sep="", header = FALSE,
                    col.names = c('chrom', 'm_start', 'm_stop', 'met_type', 'total_reads', 'met_dir', 'met_st_1', 'met_st_2', 'color','reads_all', 'methyl', 'methyl_reads', 'non_methyl_reads',
                                  'v1','v2','v3', 'v4', 'v5', 'chrom2', 'site_start', 'site_stop', 'site', 'v6', 'dir',
                                  'chrom3', 'f_start', 'f_stop', 'f_dir','f_type','locus_tag','id1','id2','locus_tag1','id11','id21','d_chrom','d_start', 'd_stop','d_old','d_id','domain'))
data_12 <- read.csv(paste(working_dir,"LSP-2.GATCAG.bed", sep="/"), sep="", header = FALSE,
                    col.names = c('chrom', 'm_start', 'm_stop', 'met_type', 'total_reads', 'met_dir', 'met_st_1', 'met_st_2', 'color','reads_all', 'methyl', 'methyl_reads', 'non_methyl_reads',
                                  'v1','v2','v3', 'v4', 'v5', 'chrom2', 'site_start', 'site_stop', 'site', 'v6', 'dir',
                                  'chrom3', 'f_start', 'f_stop', 'f_dir','f_type','locus_tag','id1','id2','locus_tag1','id11','id21','d_chrom','d_start', 'd_stop','d_old','d_id','domain'))
...
site_jitter<-ggplot(reordered_data[reordered_data$total_reads>1&reordered_data$met_type==meth_type,], 
  ...
  scale_x_discrete(labels=c("0" = "G", "1" = "A", "2" = "T", "3" = "C", "4" = "A", "5" = "G")) +
...
heat_all <-ggplot(data_all_methyl_a_samples_pv_wide_wa, aes(MEP_wt_mean,LSP_wt_mean)) + 
  stat_bin2d(bins=32) + 
  ylim(0,110) +
  scale_fill_gradientn(colours=r_2, breaks = c(10,50,90),labels=c(10,50, 90)) +
  ...
  theme(
    ...
    plot.margin=unit(c(10,10,10,20), "pt")
...
ggsave(filename = paste(site,"_figure_S2.2.CD.tiff", sep=""), plot =figure, path = results_dir,
       scale = 1, width = 180,
       height = 78, units = c("mm"),
       dpi = 300, bg = "white")
ggsave(filename = paste(site,"_figure_S2.2.CD.png", sep=""), plot =figure, path = results_dir,
       scale = 1, width = 180,
       height = 78, units = c("mm"),
       dpi = 300, bg = "white")
