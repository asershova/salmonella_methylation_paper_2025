...
gatc_methyl <- read.csv(paste(working_dir,"ATGCAT_data_all_flat_table_weight_av.csv", sep="/"), sep="", header = TRUE)
...
write.csv(all_pos_transcript_total, file = paste(results_dir, "Suppl_Table_ATGCAT_all_methyl_gene_expression.txt", sep=""),
          sep= ",", row.names = FALSE)
...
write.csv(unique(gatc_methyl[gatc_methyl$locus_tag%in%um_gatc_list,]$id2), file = paste(results_dir, "ATGCAT_UM_upstream.txt", sep=""),
          sep= "\n", row.names = FALSE)
...
write.csv(unique(gatc_methyl[gatc_methyl$locus_tag%in%
            inter_05_methyl_lsp$affected_lt,]$id2), file = paste(results_dir, "ATGCAT_DM_upstream_id.txt", sep=""),
          sep= "\n", row.names = FALSE)
...
chi_inter_wide<-lsp_mep_igb_pv%>% 
  pivot_wider(names_from = DEG, 
              values_from = n)
chi_inter_wide
m_i_o = matrix(c(12,9,3084,1825), ncol=2)
m_i_gb = matrix(c(12,9,57,45), ncol=2)
m_gb_o=matrix(c(57,45,3084,1825),ncol=2)
...
stacked_bar<-ggplot(lsp_mep_igb_pv, 
  ...
  scale_x_discrete(labels= c("Upstream\nn=21", "Gene\nn=102","Other\nn=4909"))+
  ggsignif::geom_signif(annotations = c(0.18,0.76,1), comparisons=my_comparisons_igb, 
                        y_position=c(-152.2, -152.4, -152.6),
...
ggsave(filename = paste(site,"_transcriptomic_figure_7.tiff", sep=""), plot =figure, path = results_dir,
       scale = 1, width = 180,
       height = 156, units = c("mm"),
       dpi = 300, bg = "white")
ggsave(filename = paste(site,"_transcriptomic_figure_7.png", sep=""), plot =figure, path = results_dir,
       scale = 1, width = 180,
       height = 156, units = c("mm"),
       dpi = 300, bg = "white")
