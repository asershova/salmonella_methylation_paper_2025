...
status_pal_2 <- c("#FF9896FF","#9467BDFF","#1F77B4FF")
working_dir = "/app/data"
results_dir = "/app/results"
data_08 <- read.csv(paste(working_dir,"LSP-1.CCWGG.bed", sep="/"), sep="", header = FALSE,
...
data_09 <- read.csv(paste(working_dir,"MEP-1.CCWGG.bed", sep="/"), sep="", header = FALSE,
                    col.names = c('chrom', 'm_start', 'm_stop', 'met_type', 'total_reads', 'met_dir', 'met_st_1', 'met_st_2', 'color','reads_all', 'methyl', 'methyl_reads', 'non_methyl_reads',
                                  'v1','v2','v3', 'v4', 'v5', 'chrom2', 'site_start', 'site_stop', 'site', 'v6', 'dir',
                                  'chrom3', 'f_start', 'f_stop', 'f_dir','f_type','locus_tag','id1','id2','locus_tag1','id11','id21','d_chrom','d_start', 'd_stop','d_old','d_id','domain'))
data_10 <- read.csv(paste(working_dir,"MEP-2.CCWGG.bed", sep="/"), sep="", header = FALSE,
                    col.names = c('chrom', 'm_start', 'm_stop', 'met_type', 'total_reads', 'met_dir', 'met_st_1', 'met_st_2', 'color','reads_all', 'methyl', 'methyl_reads', 'non_methyl_reads',
                                  'v1','v2','v3', 'v4', 'v5', 'chrom2', 'site_start', 'site_stop', 'site', 'v6', 'dir',
                                  'chrom3', 'f_start', 'f_stop', 'f_dir','f_type','locus_tag','id1','id2','locus_tag1','id11','id21','d_chrom','d_start', 'd_stop','d_old','d_id','domain'))
data_12 <- read.csv(paste(working_dir,"LSP-2.CCWGG.bed", sep="/"), sep="", header = FALSE,
                    col.names = c('chrom', 'm_start', 'm_stop', 'met_type', 'total_reads', 'met_dir', 'met_st_1', 'met_st_2', 'color','reads_all', 'methyl', 'methyl_reads', 'non_methyl_reads',
                                  'v1','v2','v3', 'v4', 'v5', 'chrom2', 'site_start', 'site_stop', 'site', 'v6', 'dir',
                                  'chrom3', 'f_start', 'f_stop', 'f_dir','f_type','locus_tag','id1','id2','locus_tag1','id11','id21','d_chrom','d_start', 'd_stop','d_old','d_id','domain'))
...
data_all_plus <- data_all[data_all$met_dir=="+"&data_all$dir=="+"&data_all$met_type=="m",]
data_all_minus <- data_all[data_all$met_dir=="-"&data_all$dir=="-"&data_all$met_type=="m",]
...
site_jitter<-ggplot(reordered_data[reordered_data$total_reads>1&reordered_data$met_type==meth_type,], 
  ...
  scale_x_discrete(labels=c("0" = "C", "1" = "C", "2" = "W", "3" = "G", "4" = "G")) +
  theme_ipsum(base_family = 'Arial', base_size = 12) +
  theme(
    axis.title.x = element_blank(), #text(size = 12, vjust = 0.5, hjust = 0.5),
    axis.text.x = element_text(size = 12, vjust = 0.5, hjust = 0.5, color= c("black", "#D62728FF", "black", "black", "black")),
    axis.title.y = element_text(size = 12, vjust = 0.5, hjust = 0.5),
    plot.margin=unit(c(10,0,0,10), "pt"),
  )
...
#  var1.amount var1.mean.time var2.amount var3.mean.time  wt_mean
#1          88            312          75            761 518.5951
...
write.table(data_all_methyl_a_samples_pv_wide_wa2, file = paste(working_dir,"/",site,"_data_all_flat_table_weight_av.csv",sep=""),
            sep = "\t", row.names = F)
...
stacked_bar<-ggplot(summary_1_1, 
  ...
  ggsignif::geom_signif(annotations = c(p_chi_cl,p_chi_cu), 
                        comparisons=my_comparisons, tip_length=0.000002, y_position=c(-1074.3, -1074.1),
                        textsize=3, size=0.2)+
  ...
  theme(
    axis.title.y = element_text(size = 12, vjust = 0.8, hjust = 0.5),
    axis.title.x = element_blank(), #text(size = 12, vjust = 0.8, hjust = 0.5),
...
ggsave(filename = paste(site,"_fig_4.tiff", sep=""), 
       plot =figure, 
       path = results_dir,
       scale = 1, width = 180,
       height = 156, units = c("mm"),
       dpi = 300, bg = "white")
ggsave(filename = paste(site,"_fig_4.png", sep=""), 
       plot =figure, 
       path = results_dir,
       scale = 1, width = 180,
       height = 156, units = c("mm"),
