...
gatc_methyl <- read.csv(paste(results_dir, "GATC_data_all_flat_table_weight_av.csv",sep = "/"), sep="", header = TRUE)
...
methyl_dir_pv_wide <- methyl_dir_pv %>%
  pivot_wider(names_from = methyl_status, 
              values_from = n)
write.table(methyl_dir_pv_wide, file = paste(results_dir,"/",site,"_intergenic_pos_dir.csv",sep=""),
            sep = "\t", row.names = F)
...
write.csv(all_pos_transcript_total, file = paste(results_dir, "Suppl_Table_GATC_all_methyl_gene_expression.txt", sep="/"),
          sep= ",", row.names = FALSE)
...
write.csv(unique(gatc_methyl[gatc_methyl$locus_tag%in%um_gatc_list,]$id2), file = paste(results_dir, "GATC_UM_upstream.txt", sep="/"),
          sep= "\n", row.names = FALSE)
...
write.csv(unique(gatc_methyl[gatc_methyl$locus_tag%in%
            inter_05_methyl_lsp$affected_lt,]$id2), file = paste(results_dir, "GATC_DM_upstream_id.txt", sep="/"),
          sep= "\n", row.names = FALSE)
...
violins_dm<-ggplot(lsp_mep, aes(y=log2FoldChange, x=factor(DM_sites_i_gb,
  ...
  theme(
    axis.title.y = element_blank(), #text(size = 10, vjust = 0.8, hjust = 0.5),
